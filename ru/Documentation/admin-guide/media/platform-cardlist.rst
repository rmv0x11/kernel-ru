...
Существует несколько драйверов, ориентированных на поддержку
функциональности, которая уже встроена в основную плату, и которые не
используют ни шину USB, ни шину PCI. Такие драйверы называются
платформенными (platform) и очень популярны на встраиваемых устройствах.

Поддерживаемые в настоящее время платформенные драйверы (не считая драйверов
из staging) перечислены ниже
