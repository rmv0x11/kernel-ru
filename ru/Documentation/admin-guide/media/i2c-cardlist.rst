...
Шина I²C (Inter-Integrated Circuit) — это трёхпроводная шина, используемая
внутри медиакарт для связи между различными микросхемами. Хотя сама шина не
видна ядру Linux, драйверам требуется отправлять и принимать команды по этой
шине. Абстракция драйверов ядра Linux поддерживает реализацию отдельных
драйверов для каждого компонента внутри шины I²C, как если бы шина была видна
основной системной плате.

Одна из проблем с устройствами I²C состоит в том, что иногда одно и то же
устройство может работать с разным аппаратным обеспечением I²C. Это
распространено, например, для устройств, поставляемых с одним тюнером для рынка
Северной Америки и другим — для Европы. У некоторых драйверов есть параметр
modprobe ``tuner=``, позволяющий указать другой номер тюнера, чтобы решить эту
проблему.

Поддерживаемые на данный момент драйверы I²C (без учёта драйверов из staging)
перечислены ниже.

Аудиодекодеры, процессоры и микшеры
-----------------------------------
