...
Платы PCI идентифицируются по идентификатору, называемому PCI ID. На самом деле
PCI ID состоит из двух частей:

	- Vendor ID и device ID;
	- Subsystem ID и Subsystem device ID;

Команда ``lspci -nn`` позволяет определить PCI ID вендора/устройства:
...
Subsystem ID можно получить с помощью ``lspci -vn``
...
В приведённом выше примере первая карта использует драйвер ``saa7134`` и имеет
PCI ID вендора/устройства, равный ``1131:7133``, и PCI subsystem ID, равный
``1461:f01d`` (см. :doc:`Список карт Saa7134<saa7134-cardlist>`).

К сожалению, иногда один и тот же PCI subsystem ID используется разными
продуктами. Поэтому некоторые медиадрайверы позволяют передавать параметр
``card=``, чтобы задать номер карты, который соответствовал бы правильным
настройкам для конкретной платы.

Поддерживаемые на данный момент карты PCI/PCIe (не включая драйверы из staging)
перечислены ниже\ [#]_.
...
Некоторые из этих драйверов поддерживают несколько устройств, как показано в
списках карт ниже:
