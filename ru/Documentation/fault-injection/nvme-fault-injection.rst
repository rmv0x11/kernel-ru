Внедрение сбоев NVMe
====================
Фреймворк внедрения сбоев (fault injection) ядра Linux предоставляет
систематический способ поддержки внедрения ошибок через debugfs в каталоге
/sys/kernel/debug. Когда внедрение включено, в nvme_try_complete_req будет
внедрён код по умолчанию NVME_SC_INVALID_OPCODE без повторной попытки. Пользователи
могут изменить код состояния по умолчанию и флаг отсутствия повторной попытки
через debugfs. Список Generic Command Status можно найти в include/linux/nvme.h

Следующие примеры показывают, как внедрить ошибку в nvme.

Сначала включите конфигурацию ядра CONFIG_FAULT_INJECTION_DEBUG_FS,
перекомпилируйте ядро. После загрузки ядра выполните
следующее.

Пример 1. Внедрение кода состояния по умолчанию без повторной попытки
---------------------------------------------------------------------
...
Пример 2. Внедрение кода состояния по умолчанию с повторной попыткой
--------------------------------------------------------------------
...
Пример 3. Внедрение ошибки в 10-ю административную команду
----------------------------------------------------------
